...
function filterByRegion(regionScale, regionName){
    const selectedRegion = allData.filter(f =>properties.regionScale === regionName)
...
function countCategoreis (features, selected_region) {
    ...
    const condition_demolished = selected_data.filter(f =>properties.condition === "Demolished")
    const condition_
    const coundition_count = len(filtered_condition)
    const filtered_typology = selected_data.filter(f =>properties.typology === "")
}